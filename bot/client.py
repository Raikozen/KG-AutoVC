...
class Bot(commands.Bot):
    def __init__(self):
        ...
        self.channels = data.List("./data/voicechannels.json")
        self.textchannels = data.List("./data/textchannels.json")
    ...
    async def on_voice_join(self, member, channel):
        ...
        newchannelrole = await member.guild.create_role(
            name=f"VC {new_channel.id}"
        )
        ...
        text_overwrites ={
            ...
            member.guild.get_role(newchannelrole.id): discord.PermissionOverwrite(
                view_channel=True
            ),
            member.guild.get_role(voiceadmin.id): discord.PermissionOverwrite(
                view_channel=True
            )
        }
    ...
    async def on_voice_leave(self, member, channel):
        if channel.id in self.channels:
            if len(channel.members) == 0:
                # Remove the VC role from the user
                for role in member.roles:
                    if role.name.startswith("VC"):
